...
This document explains the development workflow for svg2fbf using `just` - a modern cross-platform task runner.
...
| `just add <pkg>` | Add a runtime dependency to pyproject.toml (run `just sync` after) |
| `just add-dev <pkg>` | Add a development dependency to pyproject.toml (run `just sync` after) |
| `just remove <pkg>` | Remove a dependency from pyproject.toml (run `just sync` after) |
...
### Why svg2fbf is NOT in venv

svg2fbf is distributed as a **uv tool** (installed globally), not as an editable package in a venv. This means:
...
1. **Always use `just sync`** after adding/removing dependencies
   ```bash
   just add requests
   just sync
   ```
